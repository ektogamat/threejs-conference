import { Break, Continue, Fn, If, Loop, abs, bool, cross, distance, div, dot, float, floor, getScreenPosition, getViewPosition, int, logarithmicDepthToViewZ, luminance, max, min, mix, mod, mul, nodeObject, normalize, orthographicDepthToViewZ, passTexture, perspectiveDepthToViewZ, reference, reflect, sub, texture, trunc, uniform, uv, vec2, vec3, vec4, viewZToPerspectiveDepth } from 'three/tsl';
...
import { ENV_RAY_LENGTH, getSpecularDominantFactor, ggxReflectionSample } from 'three/addons/tsl/utils/SpecularHelpers.js';
...
import { HiZDepthPyramid } from './createHiZDepthPyramid.js';
 ...
 */
class OptimizedSSRNode extends TempNode {

	static get type() {

		return 'OptimizedSSRNode';
	...
	constructor( colorNode, depthNode, normalNode, options = {} ) {
		...
		/**
		 * Performance extensions (Threepipe-style cost architecture).
		 */
		this.classificationEnabled = uniform( 1 );
		this.metalnessFloor = uniform( 0.05 );
		this.roughnessCutoff = uniform( 0.65 );
		this.glossinessFloor = uniform( 0.08 );
		this.specularWeightFloor = uniform( 0.12 );
		this.checkerboardEnabled = uniform( 0 );
		this.frameIndex = uniform( 0 );
		this.roughnessAdaptiveEnabled = uniform( 1 );
		this.roughAdaptiveMinFactor = uniform( 0.35 );
		this.useHiZ = true;
		this._hiZPyramid = new HiZDepthPyramid( 5 );
		this._hiZPyramid._ensureLevels( 1, 1 );
		this._sourceDepthTexture = null;
		this._hiZTextureNodes = [];
	...
	setSourceDepthTexture( depthTexture ) {

		this._sourceDepthTexture = depthTexture;
	...
	updateBefore( frame ) {
		...
		this.frameIndex.value = ( this.frameIndex.value + 1 ) % 1024;
		this.frameIndex.needsUpdate = true;

		if ( this.useHiZ && this._sourceDepthTexture ) {

			const hiZWidth = Math.round( this.resolutionScale * size.width );
			const hiZHeight = Math.round( this.resolutionScale * size.height );
			this._hiZPyramid.update( renderer, this._sourceDepthTexture, hiZWidth, hiZHeight );
	...
	setup( builder ) {
		...
		const hiZCoarseNode = this.useHiZ && this._hiZPyramid.getTexture( 2 )
			? texture( this._hiZPyramid.getTexture( 2 ) )
			: null;

		const sampleMarchDepth = ( uvCoord ) => {

			return sampleDepth( uvCoord );
		...
		const ssr = Fn( () => {
			...
			if ( this.stochastic === true ) {
				...
				If( this.classificationEnabled.greaterThan( 0 ), () => {

					const dielectricRough = metalness.lessThan( this.metalnessFloor ).and( roughness.greaterThan( this.roughnessCutoff ) );
					const specWeight = max( metalness, glossiness );
					const weakSpecular = glossiness.lessThan( this.glossinessFloor ).or( specWeight.lessThan( this.specularWeightFloor ) );
					dielectricRough.or( weakSpecular ).discard();

				} );

				If( this.checkerboardEnabled.greaterThan( 0 ), () => {

					const pixel = floor( uvPos.mul( this._resolution ) );
					const parity = mod( pixel.x.add( pixel.y ).add( this.frameIndex ), 2 );
					parity.greaterThan( 0 ).discard();
			...
			const roughFactor = this.roughnessAdaptiveEnabled.greaterThan( 0 )
				? mix( this.roughAdaptiveMinFactor, float( 1 ), glossiness )
				: float( 1 );
			const effectiveQuality = this.quality.mul( roughFactor );

			const totalStep = int( this.stochastic === false
				? trunc( max( abs( xLen ), abs( yLen ) ).mul( effectiveQuality.clamp() ) ).max( int( 1 ) ).toConst()
				: effectiveQuality.clamp().mul( MAX_STEPS ).max( float( 1 ) ) ).toConst();
			...
			Loop( { start: int( 1 ), end: totalStep }, ( { i } ) => {
				...
				const d = sampleMarchDepth( uvS ).toVar();
				const vZ = getViewZ( d ).toVar();

				const viewReflectRayZ = reflectRayZAt( s ).toVar();

				if ( this.useHiZ && hiZCoarseNode ) {

					If( mod( float( i ), 4 ).equal( 0 ), () => {

						const coarseDepth = hiZCoarseNode.sample( uvS ).r;
						const coarseViewZ = getViewZ( coarseDepth );
						If( viewReflectRayZ.lessThan( coarseViewZ.sub( this.thickness.mul( 2 ) ) ), () => {
			...
			If( foundHit, () => {
				...
				if ( this.binaryRefine ) {
					...
					Loop( { start: int( 0 ), end: int( 8 ), type: 'int', condition: '<' }, () => {
						...
						const sceneZMid = getViewZ( sampleMarchDepth( screenPosAt( sMid ).mul( invResolution ) ) );
					...
					hitD.assign( sampleMarchDepth( hitUvS ) );
...
export default OptimizedSSRNode;

/**
 * Optimized SSR node with classification, motion budget hooks, checkerboard,
 * roughness-adaptive step count, and Hi-Z coarse skipping.
 */
export const optimizedSsr = ( colorNode, depthNode, normalNode, options = {} ) => nodeObject( new OptimizedSSRNode(
